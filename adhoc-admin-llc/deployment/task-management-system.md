...
1. Log in to Squarespace → Domains → DNS Settings
2. Delete any existing A records for `@` and `www`
3. Add a new A record: **Host:** `@` — **Value:** `your-vps-ip` — **TTL:** `3600`
4. Add a new A record: **Host:** `www` — **Value:** `your-vps-ip` — **TTL:** `3600`
5. Wait for DNS to propagate (can take up to 30 min — check at dnschecker.org)
